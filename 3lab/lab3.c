...
static void print_content(const char *filename);
/* -------------------------------------------------------------------------------- */
...
static void print_content(const char *filename){
  /* Zapytanie. */
  char buffer[BUFSIZE];
  printf("\nDo you want to read from '%s'? (y/n) ", filename);
  if(scanf("%s", buffer) != 1)
    return;
  ...
  if(buffer[0] == 'y' || buffer[0] == 'Y'){
    ...
    /* Inicjalizacja bloku kontrolnego. */
    struct aiocb cb;
    ...
    /* Petla do wczytywania kolejnych fragmentow pliku. */
    while(1){

      if(aio_read(&cb) == -1){
        fprintf(stderr, "Cannot create a request.\n");
        close(fd);
        return;
      }

      while(aio_error(&cb) == EINPROGRESS){/* oczekuje na dane... */}
      int nbytes = aio_return(&cb);
      buffer[nbytes] = '\0';

      if(!nbytes){
        /* Zadnych nowych danych - koniec pliku. */
        break;
      }else if(nbytes > 0){
        /* Wyswietl dane z pliku. Kazdy osobno wczytany fragment oddzielony "Read: " dla czytelnosci. */
        printf("Read: %s\n", buffer);
