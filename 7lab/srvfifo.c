  ...
  while(!bye_flag)
    {
      ...
      bye_flag = !strcmp(msg.data, "bye");
      p_pos = msg.data;
      respond(msg.data);
      strcpy(msg.data, response_str);
      printf("Your responce: %s\n", msg.data);
...
void respond (char *s)
{
    ...
    if (strlen(s)<VERYSHORT && strcmp(s,"bye"))  {
       if (find_topic(t))  {
          sprintf (response_str, "You just said:");
          sprintf (response_str, "%s\n",t);
          sprintf (response_str, "tell me more .\n");
       }
       else {
    ...
    if (in_topics(s))  {
       sprintf (response_str, "Stop repeating yourself !\n");
       return ;
    }
    ...
    if (strlen(s)>SHORT)
       sprintf (response_str, "It's seem intersting , tell me more ...\n");
    else sprintf (response_str, "Tell me more ...\n");
}
