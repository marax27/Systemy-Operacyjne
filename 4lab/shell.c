...
int executecmds(struct cmdlist* __head)
{
  ...
  struct cmdlist* curr = __head;
  pid_t pt;
  int procres;  /* == 0 if last action failed; == 1 on success */
 
  while(curr != NULL){
    /* 1.3 */
    if(curr->argv[0] != NULL){
      if(!strcmp(curr->argv[0], "exit"))
        exit(0);
    }

    /* 1.7 */
    if( !(curr->conjuction == CONJOR && procres) && !(curr->conjuction == CONJAND && !procres) ){
      f = fork();
      e = errno;
